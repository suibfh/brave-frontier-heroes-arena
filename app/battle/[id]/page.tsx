...
type BattleResult = {
  ...
  battle_key: string;
  attackerTakenDamage: number;
  defenderTakenDamage: number;
  playerName: string;
  opponentName: string;
};
            ...
            {selectedUnits.length === 0 ? (
              ...
              <div className="space-y-3">
                {selectedUnits.map((u, unitIdx) => {
                  const meta = heroMetaMap[u.heroId];
                  return (
                    <div key={u.heroId} className="flex items-center gap-3 p-3 bg-neutral-50 border border-neutral-200 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-2 mt-1">
                          {[0, 1].map((slotIdx) => {
                            const sId = u.sphereIds[slotIdx];
                            const sMeta = sId ? sphereMetaMap[sId] : null;
                            return (
                      ...
                      {/* 外すボタン */}
                      <button onClick={() => toggleUnit(u.heroId)} className="text-neutral-300 hover:text-red-500 font-bold text-lg flex-shrink-0">
                        ×
                      </button>
                    </div>
                  );
